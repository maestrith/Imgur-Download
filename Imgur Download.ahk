...
class Imgur{
	...
	Refresh(){
		refreshtoken:
		body:=imgur.info("refresh_token","client_id","client_secret") "&grant_type=refresh_token"
		this.store(iq:=imgur.http({verb:"POST",url:"https://api.imgur.com/oauth2/token",body:body}))
		return
	}
	http(info){
		static http
		...
		/*
			SetTitle("Contacting Imgur - Downloading XML for " sub " : Please Wait...")
		*/
		if(!IsObject(http))
			http:=ComObjCreate("WinHttp.WinHttpRequest.5.1")
		http.Open(info.verb,info.url,1)
	...
	UpdateAlbums(){
		...
		web.Visible:=1
		web.navigate("https://api.imgur.com/oauth2/authorize?response_type=pin&client_id=" this.client_id)
		ComObjConnect(web,"web_")
		While(web.readyState!=4||web.document.readyState!= "complete"||web.busy)
			sleep 100
		web.document.body.style.overflow:="auto"
		return
